...
const Community = () => {
  ...
  return (
    <div className='p-6 pt-12 xl:px-12 2xl:px-20 w-full mx-auto h-full overflow-y-scroll'>
      <h2 className='text-xl font-semibold mb-6 text-gray-600'>Community Images</h2>
        ...
        <div className='flex flex-wrap max-sm:justify-center gap-5'>
          {images.map((item, index) => (
            <a
              ...
              target='_blank'
              rel='noreferrer'
              className='relative group block rounded-lg overflow-hidden border border-gray-200 dark:border-purple-700 shadow-sm hover-md transition-shadow duration-300'
            >
              <img
                src={item.imageUrl}
                alt=''
              ...
              <p className='absolute bottom-0 right-0 text-xs bg-black/50 backdrop-blur text-white px-4 py-1 rounded-tl-xl opacity-0 group-hover:opacity-100 transition duration-300'>
                Created by vivek{item.username}
              </p>
            </a>
          ))}
        </div>
